...
- **桌面 beta**：`v0.2.1` 提供 Windows、macOS 和 Linux 的未签名纯二进制版本，并带有应用内版本检查。
...
当前使用 `0.x` beta 版本号。`0.1.3` 补强了 beta 存储提示、导入/导出夹具回归测试和发布验证。`0.2.0` 聚焦桌面壳、全平台二进制发布配置、引导语言选择、按章节显示的便笺/分组、关系线渲染修复，以及可调整的白板标注。`0.2.1` 增加 Settings 中的版本检查，以及用于快速初始化案件的单本 JSON 导入。
