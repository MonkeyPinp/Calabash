...
[在线 Demo](https://guesswhat-studio.github.io/Calabash/) · [反馈问题](https://github.com/Guesswhat-Studio/Calabash/issues/new/choose) · 版本 `0.3.1`
...
- **桌面 beta**：`v0.3.1` 提供未签名 Windows/Linux 二进制和 macOS DMG 镜像，并带有应用内版本检查。
...
当前使用 `0.x` beta 版本号。`0.1.3` 补强了 beta 存储提示、导入/导出夹具回归测试和发布验证。`0.2.0` 聚焦桌面壳、全平台二进制发布配置、引导语言选择、按章节显示的便笺/分组、关系线渲染修复，以及可调整的白板标注。`0.2.1` 增加 Settings 中的版本检查，以及用于快速初始化案件的单本 JSON 导入。`0.2.2` 增加日文 UI、日文 README/SEO 信息，并本地化教程 demo，尤其是金田一案例。`0.3.0` 增加按章节显示的插图、剪贴板粘贴、背景分层，以及案卷风格 Settings 重设计。`0.3.1` 修复窄宽度白板的顶部栏，让书名和右侧检查器按钮保持可见。
