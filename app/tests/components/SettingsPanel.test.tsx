...
describe('SettingsPanel', () => {
  ...
  it('keeps About focused on release links without the built-on row', () => {
    ...
    expect(screen.getByText('0.5.6')).toBeInTheDocument();
  ...
  it('checks GitHub releases and opens the matching update asset', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ...
      json: async () => [{
        tag_name: 'v0.5.7',
        html_url: 'https://github.com/Guesswhat-Studio/Calabash/releases/tag/v0.5.7',
        draft: false,
        assets: [
          { name: 'Calabash_0.5.7_windows_x64.exe', browser_download_url: 'https://example.com/calabash.exe' },
        ],
      }],
    }));
    ...
    await waitFor(() => expect(screen.getByText('Download 0.5.7')).toBeInTheDocument());

    fireEvent.click(screen.getByText('Download 0.5.7'));
