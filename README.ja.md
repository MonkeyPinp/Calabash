...
[ライブ demo](https://guesswhat-studio.github.io/Calabash/) · [Issue を報告](https://github.com/Guesswhat-Studio/Calabash/issues/new/choose) · バージョン `0.5.7`
...
- **ボード安定性・時間レイヤー・書き出し・インパクト**: `v0.5.5` は Help をクリックで開く方式にし、Auto-layout の undo/redo、時間レイヤーと『七回死んだ男』ループ demo、タブレット向けレイヤーサムネイル、PNG/PDF ボード書き出し、スマホ Settings のレスポンシブ表示、README のインパクトスナップショットを追加します。
- **デスクトップ信頼性の小改善**: `v0.5.6` は `L` で選択項目をロック/解除でき、アップロードしたポートレートを JPEG 圧縮し、デスクトップのボード画像書き出し失敗時にローカル診断ログを書きます。
- **デスクトップ書き出し hotfix**: `v0.5.7` はネイティブ保存ダイアログ経由の PNG/PDF ボード書き出しで、バイナリファイルを書き込めない権限問題を修正します。
